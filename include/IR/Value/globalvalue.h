...
namespace IR
{
    ...
    {
        Function *parent;
        int argNo = -1; // -1 is not parameter, >=0 is parameter index
        Argument(pType type, std::string name) : Value(type, name, ValueTy::ArgumentVal) {}
        Argument(pType type, int argNum) : Value(type, ValueTy::ArgumentVal), argNo(argNum) {
